<template>
  <div>
    ...
    <el-table :data="totalDatas" style="width: 100%">
      ...
      <el-table-column prop="description" label="分类简介" width="100">
      </el-table-column>
...
<script>
import { deleteById2 } from "@/api/YQgl/zixu";
...
export default {
  ...
  methods: {
    ...mapActions("zizu", ["findAll", "saveOrUpdate2", "deleteById2"]),
    //保存
    toSave() {
      this.saveOrUpdate2({
        form: this.form,
    ...
    //新增
    toAdd() {
      (this.visible = true), (this.title = "新增录入信息");
    },
    //修改
    handleClick(row) {
      this.form = { ...row };
